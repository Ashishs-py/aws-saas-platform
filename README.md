...
docs/runbooks/  Operational procedures for the five situations that page someone
```

No account ID, customer name or region appears anywhere in `modules/` or `stack/`. The account ID comes from `aws_caller_identity` at plan time; everything else is a variable with a validated value.
...
```bash
cd bootstrap
cp terraform.tfvars.example terraform.tfvars   # set org prefix, state region, GitHub owner and repo
...
**Fargate Spot in non-production.** Roughly seventy percent cheaper, and an interrupted development task carries no service impact.
